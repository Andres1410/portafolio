...
export const VideoControls = ({
	...
	return (
		//code js:
		<div
			className={`absolute bottom-0 left-0 w-full p-4 flex 
			items-center bg-whithe bg-opacity-75 ${
				isPlaying ? 'hidden group-hover:flex' : ''
			}`}
		>
			<div className=' flex items-center justify-between gap-3 w-full'>
				{/*botones play y demas: */}
				<button className='text-white focus:outline-none' onClick={togglePlay}>
					{isPlaying ? (
						<BsPauseFill size={24} />
					) : (
						<BsFillPlayFill className='bg-white' size={24} />
					)}
				</button>
				...
				<button
					className='text-white focus:outline-none bg-white'
